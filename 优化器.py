...
model = Sequential()
#加上200个神经元的隐藏层
model.add(Dense(input_dim=784,units=200,bias_initializer='one',activation='tanh',kernel_regularizer=l2(0.0003)))
#再加上100个神经元的隐藏层
model.add(Dense(input_dim=200,units=100,bias_initializer='one',activation='tanh',kernel_regularizer=l2(0.0003)))
#偏置值1；激活函数softmax-输出转成概率值
model.add(Dense(input_dim=100,units=10,bias_initializer='one',activation='softmax',kernel_regularizer=l2(0.0003)))

#定义优化器
sgd = SGD(lr=0.2)
adam = Adam(lr=0.001)#默认0.001
